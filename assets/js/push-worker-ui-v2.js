(function(){
  ...
  function replaceWorkerCopy(){
    ...
    if(detail){
      const next=(detail.textContent||'').replace(/worker GitHub/gi,'worker TrueNAS').replace(/può subire ritardi; riprova tra qualche minuto/gi,'non ha ancora processato la coda');
  ...
  async function refreshOnce(){replaceWorkerCopy();await applyMigrationState();}
