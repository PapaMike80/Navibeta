(function(){
  ...
  async function subscribe(profile,prefs){
    ...
    if(sub&&currentVapidVersion()!==VAPID_VERSION){try{await sub.unsubscribe();}catch(_){ }sub=null;}
  ...
  async function unsubscribe(profile){
    ...
    clearVapidVersion();
    if(agentId){try{await databaseRequest(`private/adminUpdates/pushSubscriptions/${safeKey(agentId)}/${safeKey(id)}`,{method:'DELETE'});}catch(_){ }}
    return true;
  }
  ...
  async function updatePreferences(profile,prefs){
    ...
    const sub=await localSubscription();
    if(sub&&Notification.permission==='granted'&&currentVapidVersion()===VAPID_VERSION)await saveSubscription(profile,sub,next);
    return next;
  }
