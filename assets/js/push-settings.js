(function(){
      ...
      <div class="push-settings-body" style="display:grid;gap:16px">
        ...
        <div>
          ...
          <div class="switch-row" style="margin:0;padding:12px 0 8px;border-top:1px solid #203e48;border-bottom:0">
            <div class="switch-copy"><strong>Riepilogo giornata di oggi</strong><span>Servizio, nave, equipaggio, ormeggio e rifornimento.</span></div>
          ...
          <div id="push-summary-schedule" style="margin:0 0 6px;padding:9px 12px 13px;border-bottom:1px solid #203e48;background:rgba(11,32,41,.45);border-radius:0 0 10px 10px">
            <div style="display:grid;grid-template-columns:minmax(0,1fr) auto;gap:10px;align-items:end">
              <div class="field" style="margin:0">
                <label for="push-summary-mode">Quando riceverlo</label>
                <select id="push-summary-mode">
                  <option value="previous-day">Giorno prima, a un orario scelto</option>
                  <option value="same-day">Giorno stesso, a un orario scelto</option>
                  <option value="before-service">Prima dell’inizio del servizio</option>
                </select>
              </div>
              <div class="field" id="push-summary-time-wrap" style="margin:0;min-width:112px">
                <label for="push-summary-time">Ora</label>
                <input id="push-summary-time" type="time" value="22:05" step="60">
              </div>
              <div class="field" id="push-summary-lead-wrap" style="margin:0;min-width:150px" hidden>
                <label for="push-summary-lead">Anticipo</label>
                <select id="push-summary-lead"><option value="30">30 minuti prima</option><option value="60" selected>1 ora prima</option><option value="120">2 ore prima</option></select>
              </div>
            </div>
            <p id="push-summary-schedule-copy" style="margin:8px 0 0;color:var(--muted);font-size:11px;line-height:1.4"></p>
          </div>
  ...
  const modeEl=$('push-summary-mode'),timeEl=$('push-summary-time'),leadEl=$('push-summary-lead');
  const timeWrap=$('push-summary-time-wrap'),leadWrap=$('push-summary-lead-wrap'),scheduleBox=$('push-summary-schedule'),scheduleCopy=$('push-summary-schedule-copy');
  ...
  function updateScheduleUi(){
    const mode=String(modeEl?.value||'previous-day');
    const relative=mode==='before-service';
    if(timeWrap)timeWrap.hidden=relative;
    if(leadWrap)leadWrap.hidden=!relative;
    if(scheduleBox)scheduleBox.style.opacity=prefTomorrow?.checked?'1':'.48';
    if(modeEl)modeEl.disabled=!prefTomorrow.checked;
    if(timeEl)timeEl.disabled=!prefTomorrow.checked;
    if(leadEl)leadEl.disabled=!prefTomorrow.checked;
    if(!scheduleCopy)return;
    const time=String(timeEl?.value||'22:05');
    const lead=Number(leadEl?.value||60);
    if(mode==='previous-day')scheduleCopy.textContent=`Riceverai automaticamente il riepilogo alle ${time} del giorno prima.`;
    else if(mode==='same-day')scheduleCopy.textContent=`Riceverai automaticamente il riepilogo alle ${time} del giorno stesso.`;
    else scheduleCopy.textContent=`Riceverai automaticamente il riepilogo ${lead===60?'1 ora':lead===120?'2 ore':'30 minuti'} prima dell’inizio del servizio.`;
  }

  const prefsFromUi=()=>({
    tomorrowSummary:prefTomorrow.checked,
    shiftChanges:prefChanges.checked,
    ods:prefOds.checked,
    summaryDelivery:{
      mode:String(modeEl?.value||'previous-day'),
      time:String(timeEl?.value||'22:05'),
      leadMinutes:Number(leadEl?.value||60)
    }
  });

  function fillPrefs(prefs){
    const normalized=window.NaviPush?.normalizePreferences?NaviPush.normalizePreferences(prefs):prefs||{};
    prefTomorrow.checked=normalized?.tomorrowSummary!==false;
    prefChanges.checked=normalized?.shiftChanges!==false;
    prefOds.checked=normalized?.ods!==false;
    if(modeEl)modeEl.value=normalized?.summaryDelivery?.mode||'previous-day';
    if(timeEl)timeEl.value=normalized?.summaryDelivery?.time||'22:05';
    if(leadEl)leadEl.value=String(normalized?.summaryDelivery?.leadMinutes||60);
    updateScheduleUi();
  }

  async function savePrefs(message='Preferenze notifiche salvate.'){
    try{await NaviPush.updatePreferences(profile,prefsFromUi());statusEl.textContent=message;}
  ...
  [prefTomorrow,prefChanges,prefOds].forEach(input=>input?.addEventListener('change',()=>{updateScheduleUi();savePrefs();}));
  [modeEl,timeEl,leadEl].forEach(input=>input?.addEventListener('change',()=>{updateScheduleUi();savePrefs('Orario del riepilogo automatico salvato.');}));
  ...
  async function loadAdminAgents(){
    ...
    try{
      ...
      const [users,subs]=await Promise.all([window.NaviAdminFirebase?.listRegisteredUsers?.()||[],NaviPush.listSubscriptions()]);
      const counts=new Map();subs.forEach(item=>counts.set(String(item.agentId),Number(counts.get(String(item.agentId))||0)+1));
      const byId=new Map();users.forEach(user=>{if(user?.id)byId.set(String(user.id),user);});
      ...
      adminAgent.innerHTML='<option value="">Scegli agente…</option>'+ordered.map(user=>{const count=Number(counts.get(String(user.id))||0);return `<option value="${esc(user.id)}" ${count?'':'disabled'}>${esc(user.name||user.id)} · ${count?`${count} dispositivo${count===1?'':'i'}`:'nessun dispositivo'}</option>`;}).join('');
  ...
  adminSend?.addEventListener('click',async()=>{
    ...
    try{
      await NaviPush.queuePush({requestedByAgentId:agentId,requestedByName:String(profile?.name||profile?.agente||profile?.cognome||agentId),targetAgentId,title:adminTitle.value,body:adminBody.value,url:'naviturni.html',kind:'admin-test'});
      adminStatus.textContent='✅ Notifica in coda.';
