(function(){
  ...
  function defaultPreferences(){
    return {tomorrowSummary:true,shiftChanges:true,ods:true};
  }

  function preferences(agentId){
    return {...defaultPreferences(),...(readJson(PREFS_PREFIX+String(agentId||''),{})||{})};
  }
  ...
  function readAuth(){
    try{return JSON.parse(localStorage.getItem(AUTH_KEY)||'null')||volatileAuth;}catch(_){return volatileAuth;}
  }

  function saveAuth(value){
    volatileAuth=value;
    try{localStorage.setItem(AUTH_KEY,JSON.stringify(value));}catch(_){ }
    return value;
  }
  ...
  async function registration(){
    ...
    if(!reg){
      reg=await navigator.serviceWorker.register('sw.js?push=20260905',{scope:'./',updateViaCache:'none'});
  ...
  async function subscribe(profile,prefs){
    ...
    if(sub){
      ...
      if(current&&current!==VAPID_PUBLIC_KEY){await sub.unsubscribe();sub=null;}
    }
  ...
  async function updatePreferences(profile,prefs){
    ...
    const sub=await localSubscription();
    if(sub&&Notification.permission==='granted')await saveSubscription(profile,sub,next);
    return next;
  }
  ...
  async function getStatus(profile){
    const agentId=String(profile?.id||profile?.agentId||'').trim();
    const sub=await localSubscription();
    const enabled=Boolean(sub)&&Notification.permission==='granted';
    return {supported:'Notification' in window&&'PushManager' in window,permission:'Notification' in window?Notification.permission:'unsupported',enabled,deviceId:deviceId(),preferences:preferences(agentId)};
  }
  ...
  async function queuePush(payload={}){
    ...
    const item={
      id,
      status:'pending',
      kind:String(payload.kind||'admin-test'),
      requestedByAgentId,
      requestedByName:String(payload.requestedByName||''),
      ownerUid:auth.uid,
      targetAgentId,
      title,
      body,
  ...
  window.NaviPush={
    ...
    isIos,
    isStandalone,
    provider:'Web Push + Firebase registry'
  };
})();